...
namespace Controllers
{
    ...
    {
        [Header("模块管理")] [SerializeField] private BaseModule centerModule; // 中心模块

        [SerializeField] private List<ModuleInfo> assembledModules = new(); // 所有已拼装模块信息

        [Header("网格设置")] [SerializeField] private float gridSize = 1f; // 网格大小（每格的世界坐标单位）

        [Header("攻击设置")] [SerializeField] private bool enableAttack = true; // 是否启用攻击

        [SerializeField] private LayerMask enemyLayerMask = 1 << 8; // 敌人图层
        [SerializeField] private float globalAttackRange = 10f; // 全局攻击范围

        [Header("调试信息")] [SerializeField] private bool showDebugInfo = true; // 显示调试信息
        ...
        {
            // 确保中心模块已设置
            if (centerModule == null) Debug.LogError("ScriptManager 中心模块未设置");
        ...
        private void InitializeModules()
        {
            ...
            if (showDebugInfo) Debug.Log($"[ScriptManager] 初始化完成，收集到 {assembledModules.Count} 个模块");
        }
        ...
        public void RegisterModule(BaseModule module, string customName = "")
        {
            ...
            // 使用世界坐标进行计算
            var centerPos = centerModule != null ? centerModule.transform.position : transform.position;
            var moduleInfo = new ModuleInfo(module, centerPos, gridSize, customName);
            assembledModules.Add(moduleInfo);

            if (showDebugInfo)
                Debug.Log(
                    $"[ScriptManager] 模块 {moduleInfo.moduleName} 已注册，网格偏移: ({moduleInfo.gridOffset.x}, {moduleInfo.gridOffset.y}, {moduleInfo.gridOffset.z})");
        }
        ...
        {
            var removedCount = assembledModules.RemoveAll(info => info.module == module);

            if (showDebugInfo && removedCount > 0) Debug.Log($"[ScriptManager] 模块 {module.name} 已注销");
        }
        ...
        private void CollectChildModules(BaseModule parentModule)
        {
            ...
            foreach (var childModule in parentModule.childModules)
                if (childModule != null)
                {
        ...
        {
            foreach (var moduleInfo in assembledModules)
            {
        ...
        {
            // 获取攻击范围内的目标
            var targets = attackableModule.GetTargetsInRange();
        ...
        public (Vector3Int min, Vector3Int max) GetGridBounds()
        {
            ...
            var min = assembledModules[0].gridOffset;
            var max = assembledModules[0].gridOffset;
        ...
        public void PrintGridLayout()
        {
            ...
            for (var z = bounds.max.z; z >= bounds.min.z; z--)
            {
                ...
                {
                    var moduleAtPos = GetModuleByGridOffset(new Vector3Int(x, bounds.min.y, z));
                    if (moduleAtPos != null)
                        line += "[M] ";
                    else
                        line += "[ ] ";
        ...
        public class ModuleInfo
        {
            ...
            public Vector3Int gridOffset; // 相对中心的网格偏移量 (x格,y格,z格)
